#!/usr/bin/env python3
"""
Ask-Marvin Foreground Processor UI
...
class ForegroundProcessorUI:
    def __init__(self, work_dir):
        self.work_dir = Path(work_dir)
        
        # Source files on external drive
        self.base_path = Path("/Volumes/Marvin/CyanoVerse_Source_Files")
        self.foregrounds_raw = self.base_path / "Foregrounds_Raw"
        
        # Working directories
        self.preview_dir = self.work_dir / "foreground_previews"
        self.preview_dir.mkdir(parents=True, exist_ok=True)
        
        self.processed_dir = self.work_dir / "processed_foregrounds"
        self.processed_dir.mkdir(parents=True, exist_ok=True)
        
    def find_unprocessed_folders(self):
        """Find folders without 'x' prefix"""
        unprocessed = []
        for item in self.foregrounds_raw.iterdir():
    ...
    def create_folder_selection_page(self, folders, folder_results):
        ...
        html_path = self.work_dir / "select_foreground_folder.html"
...
<head>
    ...
    <title>Select Foreground Folder</title>
    <style>
...
<body>
    <h1>🔬 Select Foreground Folder (Biocrusts)</h1>
    
    <div class="instructions">
        <h2 style="margin-bottom: 10px; color: #f92672;">Instructions:</h2>
        <ul style="margin-left: 20px;">
            <li>Select a folder to process foreground macro images</li>
            <li>These are the biocrust close-ups shot with macro lens</li>
            <li>When finished, folder will be marked complete (x prefix)</li>
        </ul>
    </div>
...
"""
        ...
        <div class="no-folders">
            <h2>✅ All foreground folders processed!</h2>
...
"""
        ...
        <button class="folder-button" onclick="selectFolder('{folder.name}')">
            ...
            <div class="folder-info">{cr2_count} macro images</div>
        </button>
"""
    ...
    <script>
        function selectFolder(folderName) {
            ...
            window.location.href = 'process_fg_' + folderName + '.html';
        }
    </script>
</body>
</html>
"""
    ...
    def create_html_viewer(self, result):
        ...
        html_path = self.work_dir / f"process_fg_{folder_name}.html"
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Process Foreground: {folder_name}</title>
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        h1 {{ text-align: center; margin-bottom: 30px; color: #f92672; }}
        ...
        .instructions h2 {{ margin-bottom: 10px; color: #f92672; }}
...
<body>
    <h1>🔬 Process Foreground Images (Biocrusts)</h1>
    ...
    <div class="instructions">
        ...
        <ul>
            ...
            <li><strong>Select 2+ images:</strong> Stacks them together (MACRO FOCUS STACK)</li>
...
"""
        ...
        for idx, img in enumerate(result['images']):
            rel_path = f"foreground_previews/{img['preview']}"
            card_id = f"card_{idx}"
            html_content += f"""
...
"""
    ...
    <script>
        ...
        function updateUI() {{
            ...
            if (count === 1) {{
                button.innerHTML = '💾 Save Selected Image<span class="selection-count" id="selectionCount">' + count + '</span>';
            }} else if (count > 1) {{
                button.innerHTML = '📚 Stack & Save ' + count + ' Images<span class="selection-count" id="selectionCount">' + count + '</span>';
            }} else {{
                button.innerHTML = '⚡ Process Selection<span class="selection-count" id="selectionCount">0</span>';
            }}
        }}
        ...
        function processSelection() {{
            ...
            alert('Selection saved! Run: python3 process_images.py');
        }}
        ...
        function markComplete() {{
            if (confirm('Mark folder "' + folderName + '" as complete?\\n\\nThis will add "x" prefix and return to folder selection.')) {{
                ...
                window.location.href = 'select_foreground_folder.html';
            }}
        }}
...
"""
    ...
    def run(self):
        """Main workflow"""
        print("Ask-Marvin Foreground Processor UI")
        print("="*60)
        
        unprocessed = self.find_unprocessed_folders()
        
        if not unprocessed:
            print("No unprocessed foreground folders found!")
            print("Creating folder selection page...")
            html_path = self.create_folder_selection_page([], {})
            os.system(f'open "{html_path}"')
            return
        
        print(f"\nFound {len(unprocessed)} unprocessed foreground folder(s):")
        ...
        print("="*60)
        print(f"\nWorkflow:")
        print(f"  1. Select a foreground folder to process")
        print(f"  2. Select/stack biocrust macro images")
        print(f"  3. Mark folder complete when done")
        print(f"  4. Process stacks with: python3 process_images.py")
...
if __name__ == "__main__":
    ...
    processor = ForegroundProcessorUI(work_dir)
    processor.run()
