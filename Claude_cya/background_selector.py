#!/usr/bin/env python3
"""
Ask-Marvin Background Image Selector
...
class BackgroundSelector:
    def __init__(self, work_dir):
        ...
        self.selections_file = self.work_dir / "selected_backgrounds.json"
    ...
    def create_html_viewer(self, all_results):
        ...
        html_path = self.work_dir / "background_selector.html"
...
<head>
    ...
    <title>Ask-Marvin Background Selector</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
<body>
    <h1>🖼️ Ask-Marvin Background Selector</h1>
    ...
    <div class="instructions">
        ...
        <ul>
            ...
            <li>If you select multiple from one folder, they'll be numbered (name_1.png, name_2.png, etc.)</li>
            <li>Click "Save Selection" when done</li>
            <li>Your selections will be saved to: <code>selected_backgrounds.json</code></li>
        </ul>
    </div>
...
"""
    ...
    <script>
        ...
        function loadSelections() {{
            ...
            const saved = localStorage.getItem('askMarvinBackgroundSelections');
            if (saved) {{
                try {{
        ...
        function toggleSelection(checkbox) {{
            ...
            localStorage.setItem('askMarvinBackgroundSelections', JSON.stringify(selectedImages));
            updateSelectionCount();
        }}
        ...
        function saveSelection() {{
            // Add numbering info for multiple selections
            const outputData = {{}};
            
            for (let folder in selectedImages) {{
                const images = selectedImages[folder];
                
                if (images.length === 1) {{
                    // Single image: use folder name as-is
                    outputData[folder] = {{
                        output_name: folder + '.png',
                        images: images
                    }};
                }} else {{
                    // Multiple images: number them
                    outputData[folder] = {{
                        output_names: images.map((img, idx) => folder + '_' + (idx + 1) + '.png'),
                        images: images
                    }};
                }}
            }}
            ...
            a.href = url;
            a.download = 'selected_backgrounds.json';
            ...
            statusMsg.textContent = 'Selection saved! Check your Downloads folder for selected_backgrounds.json';
            statusMsg.classList.add('show', 'success');
            setTimeout(() => {{
                statusMsg.classList.remove('show');
            }}, 4000);
            
            console.log('Selected images with naming:', outputData);
        }}
...
"""
    ...
    def run(self):
        """Main workflow"""
        print("Ask-Marvin Background Selector")
        print("="*60)
        ...
        print(f"  - Selector: {html_path}")
        print(f"  - Selections will be saved to: selected_backgrounds.json (Downloads)")
        print(f"\nOpening selector in browser...")
...
if __name__ == "__main__":
    ...
    selector = BackgroundSelector(work_dir)
    selector.run()
